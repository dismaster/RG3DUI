...
if [[ $(uname -o) == "Android" ]]; then
    ...
    run_command_silently pkg install -y cronie termux-services termux-auth libjansson wget nano git screen openssh termux-services libjansson netcat-openbsd jq termux-api iproute2 tsu android-tools

    # Create ~/.termux folder if not exists
    run_command_silently mkdir -p ~/.termux
    run_command_silently mkdir -p ~/.cache

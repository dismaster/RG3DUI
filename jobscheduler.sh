...
response=$(curl_request "https://gui.rg3d.eu/checkjob.php" "$post_data")
...
config_response=$(curl_request "https://gui.rg3d.eu/getconfig.php" "$post_data")
...
if [ -n "$job_id" ]; then
    complete_response=$(curl_request "https://gui.rg3d.eu/completejob.php" "job_id=$job_id")
    if [ $? -ne 0 ]; then
